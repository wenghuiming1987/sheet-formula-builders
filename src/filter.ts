import { buildComparison, cleanInput, compactCriteria, quoteFormulaText, type CriteriaPair } from "./helpers.js";
...
export function buildExcelFilterFormula(options: FilterOptions): string {
...
export function buildGoogleSheetsFilterFormula(options: FilterOptions): string {
  const dataRange = cleanInput(options.dataRange, "A2:D100");
  const conditions = compactCriteria(options.conditions);
  const include = conditions.map((condition) => buildComparison(condition.range, condition.value)).join(", ");
...
function applySheetName(range: string, sheetName?: string): string {
  const cleaned = cleanInput(range);
  if (!sheetName || cleaned.includes("!")) return cleaned;
...
function formatFilterValue(value: string, type: FilterConditionType = "text"): string {
  const cleaned = cleanInput(value);
  if (type === "number") return cleaned || "0";
  if (type === "date") {
...
function comparisonOperator(operator: FilterConditionOperator): string {
  const map: Partial<Record<FilterConditionOperator, string>> = {
    equals: "=",
    "not-equals": "<>",
    "greater-than": ">",
    "greater-than-or-equal": ">=",
...
function buildConditionExpression(condition: GoogleSheetsFilterCondition, sheetName?: string): string {
  const range = applySheetName(condition.range, sheetName);
  const type = condition.type || "text";
  if (condition.operator === "contains") return `ISNUMBER(SEARCH(${quoteFormulaText(cleanInput(condition.value))},${range}))`;
  if (condition.operator === "does-not-contain") return `NOT(ISNUMBER(SEARCH(${quoteFormulaText(cleanInput(condition.value))},${range})))`;
  if (condition.operator === "is-blank") return `${range}=""`;
  if (condition.operator === "is-not-blank") return `${range}<>""`;
  return `${range}${comparisonOperator(condition.operator)}${formatFilterValue(cleanInput(condition.value), type)}`;
}

function rangeHeight(range: string): number | null {
...
function rangeColumn(range: string): string | null {
  const cleaned = range.trim().split("!").pop() || "";
  const match = cleaned.match(/^\$?([A-Z]+)\$?\d+:\$?\1\$?\d+$/i);
  return match ? match[1].toUpperCase() : null;
}

function queryValue(condition: GoogleSheetsFilterCondition): string | null {
  const type = condition.type || "text";
  const value = cleanInput(condition.value);
  if (condition.operator === "is-blank") return "IS NULL";
  if (condition.operator === "is-not-blank") return "IS NOT NULL";
  if (condition.operator === "contains") return `contains '${value.replaceAll("'", "\\'")}'`;
  if (condition.operator === "does-not-contain") return null;
  if (type === "date") {
    const parsed = parseIsoDate(value);
    if (!parsed) return null;
    return `${comparisonOperator(condition.operator)} date '${value}'`;
  }
  if (type === "number") return `${comparisonOperator(condition.operator)} ${value}`;
  return `${comparisonOperator(condition.operator)} '${value.replaceAll("'", "\\'")}'`;
}

function buildQueryAlternative(dataRange: string, conditions: GoogleSheetsFilterCondition[], logic: "AND" | "OR"): string {
  ...
  if (whereParts.some((part) => !part)) return "Not generated for this condition set.";
  return `=QUERY(${dataRange}, "SELECT * WHERE ${whereParts.join(` ${logic} `)}", 0)`;
}

export function buildGoogleSheetsFilterGenerator(options: GoogleSheetsFilterGeneratorOptions): GoogleSheetsFilterGeneratorResult {
  ...
  const conditionHeights = conditions.map((condition) => rangeHeight(condition.range)).filter((height): height is number => height !== null);
...
export function isValidFilterIsoDate(value: string): boolean {
  return parseIsoDate(value) !== null;
}

export function filterRangeHeight(range: string): number | null {
