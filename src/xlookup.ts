...
export type XlookupOptions = {
  ...
  criteria?: Array<{ range: string; value: string }>;
};

export function buildXlookupFormula(options: XlookupOptions): string {
  const lookupValue =
    options.mode === "multiple" && options.criteria?.length
      ? "1"
  ...
  if (matchMode) {
    if (!cleanInput(options.ifNotFound)) args.push('""');
    args.push(matchMode);
  }
