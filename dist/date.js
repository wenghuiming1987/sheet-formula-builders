...
export function buildDateDifferenceFormula(startDateCell, endDateCell, unit) {
    const start = cleanInput(startDateCell, "A2");
    const end = cleanInput(endDateCell, "B2");
    if (unit === "months")
        return `=DATEDIF(${start}, ${end}, "M")`;
    if (unit === "years")
        return `=DATEDIF(${start}, ${end}, "Y")`;
    return `=${end}-${start}`;
}
export function isoDateToExcelSerial(isoDate) {
...
export function excelSerialToIsoDate(serial) {
    if (!Number.isFinite(serial) || serial < 1)
        throw new Error("Invalid Excel serial");
    if (serial === 60)
        return "1900-02-29";
