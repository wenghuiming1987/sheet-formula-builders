...
function looksLikeCellReference(value) {
    return /^\$?[A-Z]{1,3}\$?\d+$/i.test(value.trim());
}
function dateValueSegments(value) {
...
function dateConditionSegments(column, operator, value) {
...
function queryArgument(segments) {
    const compacted = [];
...
export function buildQueryFormula(options) {
    const segments = [text(`SELECT ${cleanInput(options.selectedColumns || options.selectColumns, "A, B, C")}`)];
    const whereParts = [];
    ...
    return `=QUERY(${cleanInput(options.dataRange, "A1:D100")}, ${queryArgument(segments)}, ${cleanInput(options.headerRows, "1")})`;
}
